...
- O workspace fixo é resolvido server-side; o P0 opera publicamente sem autenticação conforme a decisão vigente.
...
|    14 | E05-T06     | cancelada; risco público e workspace fixo documentados      |
...
E04-T03 pode começar após E04-T02, mas o ponteiro ativo seleciona primeiro E04-T06 para registrar a decisão. O CI precede a validação de prontidão; o shell depende da integração versionada. E06 e E07 começam após a fundação persistente de E05-T05, consomem o workspace fixo server-only e suas telas dependem também de E04-T04.

## Validação acumulada

Após E04, os scripts existentes devem cobrir instalação congelada inclusive do package privado, formato, lint, tipos, testes, cobertura, dados e build. A página foundations comprova CSS público, `@source`, Inter, dark, acessibilidade e responsividade antes do shell. Após E05, acrescentar migration em banco limpo, importação repetida, constraints, backup/restore local e testes do workspace fixo.

Verificações manuais permanentes:

- nenhum segredo, caminho local ou dado operacional no Git;
- DTO público de plano nunca contém atribuição, execução ou comentário;
- DTO operacional sempre filtra o workspace resolvido no servidor;
...
| resolução de workspace falha             | bloquear operação até corrigir a configuração server-only               |
| feature falha                            | reverter código compatível; não apagar execuções                        |
| URL pública deixa de ser risco aceitável | bloquear publicação e priorizar autenticação real antes de reabrir      |
